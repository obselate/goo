...
public sealed class TabStopTests
{
    ...
    public void ExplicitFocusableFalseOverridesInteractivePrimitiveDefaults()
    {
        ...
        finally { input.Dispose(); NodeLifecycle.DisposeTree(root); controller.Dispose(); }
    }
    ...
    public void SequentialFocusSkipsCompositeChildrenButExplicitFocusRemainsAvailable()
    {
        ...
        var text = new TextInput(focus);
        var keyboard = new KeyboardInput(focus);
        try
        {
            focus.SetFocus(resolver, root.Children[0]);
            ...
            Assert.True(root.Children[1].Focusable);
            focus.SetFocus(resolver, root.Children[1]);
        ...
        finally { focus.Dispose(); text.Dispose(); }
