...
open class CounterCell : Cell[CounterInput] {
    shared {
        let Card Style = Style{Padding: 24, Gap: 12, BorderRadius: 16, BackgroundColor: Color.Rgb(20, 27, 39),}
    ...
    protected override func Build(input CounterInput) Blob -> Container{
        ...
        Children: {
            Text{Content: input.Title + ": " + count.ToString(), FontSize: 24, Color: Color.White,},
            Button{
                BasedOn: Action,
...
class MainCell : Cell {
    override func Build() Blob -> Container{
        Children: {
            Cell.Mount[CounterInput, CounterCell](
                "counter",
...
func Main() {
    ...
    window.StateChanged += (state) -> {
        if state == WindowState.Maximized {
            window.Title = "Goo starter - maximized"
        }
    }
    window.Run()
}
