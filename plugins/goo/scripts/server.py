...
def goo_context(repository: str = "") -> dict:
    ...
    return {"source": str(root), "bundled": root == BUNDLE, "bundleCommit": MANIFEST["commit"], "documents": sorted(MANIFEST["files"]), "runtime": "Install Goo.DevTools 0.5.0 and launch with goo dev --project App.gsproj. Snapshot/capture require the app PID. GOO_CLI may specify a goo executable or built Goo.DevTools.Cli.dll."}
