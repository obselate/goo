...
`goo_snapshot` requests `full: true` on every call. Its `payload.added` contains the full current tree, with stable IDs for nodes that remain mounted. It fails on runtimes that return an incomplete tree. Current runtimes return a full tree for every remote snapshot request.

For automation, launch with `goo dev --input --no-watch --project App.gsproj`. This enables diagnostics and input without setting environment variables yourself. When launching outside the CLI, set both `GOO_DEVTOOLS=1` and `GOO_DEVTOOLS_INPUT=1`. `goo_input` uses normal hit testing, focus, event routing, and Cell updates. Calls for the same explicit target are serialized. On runtimes that advertise gesture leases, the plugin retains the token across pointer and key holds for 30 seconds and always uses it for cleanup. Supply `node_id` from the selected window snapshot or logical window coordinates. Text goes to the focused editor. Await each event, finish gestures, and inspect the resulting snapshot and image. A timed-out action may have applied and must not be retried automatically. Input is marked as state-changing and can trigger application behavior. Inspection tools remain read-only.
