...
For automation, launch with `goo dev --input --no-watch --project App.gsproj`. This enables diagnostics and input without setting environment variables yourself. When launching outside the CLI, set both `GOO_DEVTOOLS=1` and `GOO_DEVTOOLS_INPUT=1`. `goo_input` uses the existing input capability and normal hit testing, focus, event routing, and Cell updates. Supply `node_id` from the selected window snapshot or logical `x`/`y` coordinates. Text goes to the focused editor. Await each event, finish pointer gestures, and inspect the resulting snapshot and image. A timed-out action may have applied and must not be retried automatically. Input is marked as state-changing and can trigger application behavior. Inspection tools remain read-only.
