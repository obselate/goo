...
The checked-in MCP manifest is intentionally unconfigured because the tested Codex build did not expand plugin-root placeholders in MCP arguments. Run `python3 scripts/configure.py` before every first install from a new location. The script binds the manifest to that plugin source while preserving unrelated servers, server settings, and custom environment values. Run `python3 scripts/configure.py --check` to detect a moved or stale source without changing the file. Keep the configured source folder available after installation, and reinstall after moving it. Set `GOO_CLI` in the server environment if needed. No API key or hosted model service is required.
...
Call `goo_context` first and inspect its `preflight` result. It reports the loaded plugin version, source fingerprint, disk drift, manifest binding, configured CLI version and required feature contract. Runtime tools reject a missing or incompatible CLI before use. The check is cached by CLI path, modification time and size, and runtime wait budgets include a cold preflight. The loaded server can report only its own source and CLI, not definitions loaded by another session.

Call `goo_targets` and select the intended PID and stable window ID. Runtime tools require an explicit PID and reject ambiguous window selection. Discovery reads descriptors without attaching. It excludes exited processes. A window ID distinguishes windows that share a title. Snapshot results include the handshake's protocol version, explicit runtime version when supplied, capabilities, target identity, input permission, and inspect mode when supplied. Unknown values remain `unavailable` and are not inferred from protocol schema numbers or capabilities.
...
Run `python3 scripts/configure.py --check`, reinstall the local plugin after changing its source, and start a new Codex thread to load updated skills and MCP tool definitions.
