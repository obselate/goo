...
internal unsafe partial class VulkanImageResources {
  internal func MemoryCapacitySnapshotForTest() VulkanImageMemoryCapacitySnapshot {
    let allocation = stagingAllocation
    return VulkanImageMemoryCapacitySnapshot{
      ResidentBytes: residentBytes,
      StagingConfiguredBytes: stagingByteCapacity,
      StagingAllocatedBytes: allocation?.size ?? 0uL,
      StagingPlacementBytes: allocation?.placementSpan ?? 0uL,
      StagingMapped: allocation?.mapped != nil,
    }
  }
}

internal unsafe partial class VulkanPrimitiveRenderer {
  internal func PrimitiveMemoryCapacitySnapshotForTest()
  VulkanFrameMemoryCapacitySnapshot -> primitiveFrameData.MemoryCapacitySnapshotForTest()

  internal func TextMemoryCapacitySnapshotForTest()
  VulkanFrameMemoryCapacitySnapshot -> textFrameData.MemoryCapacitySnapshotForTest()

  internal func ClipMemoryCapacitySnapshotForTest() VulkanFrameMemoryCapacitySnapshot {
    if let data = clipMaskFrameData {
      return data.MemoryCapacitySnapshotForTest()
    }
...
internal partial class VulkanWindowTarget {
  internal func MemoryBlocksForTest() []VulkanMemoryBlockSnapshot {
    if let allocator = memoryAllocator {
      return allocator.SnapshotBlocks()
    }
    return [0]VulkanMemoryBlockSnapshot
  }

  internal func MemoryOwnerCapacitiesForTest() VulkanMemoryOwnerCapacitySnapshot {
    let image = imageResources?.MemoryCapacitySnapshotForTest() ?? VulkanImageMemoryCapacitySnapshot{}
    let textAtlasStats = textAtlas?.Stats ?? VulkanTextAtlasSetStats{}
    let path = pathResources
    let renderer = primitiveRenderer
    let swapchainWidth = generation?.Extent.width ?? 0u
    let swapchainHeight = generation?.Extent.height ?? 0u
    let swapchainImageCount = generation?.ImageCount ?? 0u
    let swapchainRawRgbaBytes = uint64(swapchainWidth) * uint64(swapchainHeight) * 4uL * uint64(swapchainImageCount)
    return VulkanMemoryOwnerCapacitySnapshot{
      SwapchainWidth: swapchainWidth,
      SwapchainHeight: swapchainHeight,
      SwapchainImageCount: swapchainImageCount,
      SwapchainRawRgbaBytes: swapchainRawRgbaBytes,
      ImageResidentBytes: image.ResidentBytes,
      ImageStagingConfiguredBytes: image.StagingConfiguredBytes,
      ImageStagingAllocatedBytes: image.StagingAllocatedBytes,
      ImageStagingPlacementBytes: image.StagingPlacementBytes,
      ImageStagingMapped: image.StagingMapped,
      PrimitiveFrame: renderer?.PrimitiveMemoryCapacitySnapshotForTest() ?? VulkanFrameMemoryCapacitySnapshot{},
      TextFrame: renderer?.TextMemoryCapacitySnapshotForTest() ?? VulkanFrameMemoryCapacitySnapshot{},
      ClipFrame: renderer?.ClipMemoryCapacitySnapshotForTest() ?? VulkanFrameMemoryCapacitySnapshot{},
      TextAtlasCount: textAtlasStats.AtlasCount,
      TextAtlasPerBufferBytes: textAtlasStats.ByteSize,
      PathAtlasPerBufferBytes: path?.Atlas.Stats.ByteCapacity ?? 0uL,
      PathCpuShadowBytes: uint64(path?.CpuWords.Length ?? 0) * 4uL,
      ClipAtlasResidentBytes: clipMaskAtlas?.Stats.ResidentBytes ?? 0uL,
      LayerResidentBytes: layerPool?.ResidentBytes ?? 0uL,
      LayerTargetCount: layerPool?.LiveTargetCount ?? 0,
      ReadbackResidentResourceBytes: ReadbackResidentResourceBytes,
    }
  }
...
public partial class Window {
  ...
  internal func MemoryBlocksForTest() []VulkanMemoryBlockSnapshot {
    if let target = VulkanTargetForTest() {
      return target.MemoryBlocksForTest()
    }
    return [0]VulkanMemoryBlockSnapshot
  }
  ...
  internal func MemoryOwnerCapacitiesForTest() VulkanMemoryOwnerCapacitySnapshot {
    if let target = VulkanTargetForTest() {
      return target.MemoryOwnerCapacitiesForTest()
    }
    return VulkanMemoryOwnerCapacitySnapshot{}
  }
  ...
  internal func SetFrameProfileSinkForTest(sink FrameProfileSink?) {
    profiler.Sink = sink
  }
...
internal class WindowReadbackTestFixture {
  shared {
    ...
    internal func SetFrameProfileSink(window Window, sink FrameProfileSink?) {
      window.SetFrameProfileSinkForTest(sink)
    }
    ...
    internal func MemoryBlocks(window Window) []VulkanMemoryBlockSnapshot ->
    window.MemoryBlocksForTest()
    internal func MemoryOwnerCapacities(window Window) VulkanMemoryOwnerCapacitySnapshot ->
    window.MemoryOwnerCapacitiesForTest()
    internal func TargetMemoryBlocks(target VulkanWindowTarget?) []VulkanMemoryBlockSnapshot {
      if let retained = target {
        return retained.MemoryBlocksForTest()
      }
      return [0]VulkanMemoryBlockSnapshot
    }
    internal func TargetMemoryOwnerCapacities(target VulkanWindowTarget?)
    VulkanMemoryOwnerCapacitySnapshot{
      if let retained = target {
        return retained.MemoryOwnerCapacitiesForTest()
      }
      return VulkanMemoryOwnerCapacitySnapshot{}
    }
