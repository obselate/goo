...
WORKLOADS = ['text-preset', 'layout-reflow', 'list-category', 'opening', 'image-upload', 'shader-radial-light', 'shader-dither']
ROUND_VARIANTS = {0: 'baseline', 1: 'candidate', 2: 'candidate', 3: 'baseline'}
EXPECTED_ENV = {'WAYLAND_DISPLAY': 'goo-chart-update-nested', 'SDL_VIDEODRIVER': 'wayland', 'VK_LOADER_LAYERS_DISABLE': '~implicit~', 'GOO_VK_DIAGNOSTICS': '1', 'GOO_GALLERY_REVIEW_BENCH': '1', 'GOO_GALLERY_REVIEW_PACE16MS': '1'}
...
for folder in folders:
    ...
    for path in sorted(folder.glob('*.log')):
        ...
        row = {'round': int(round_index), 'variant': variant, 'workload': path.stem, 'state': state, 'samples': 500, 'host_ms': result['host_ms'], 'frame_alloc_B': result['frame_alloc_B'], 'gpu_main_ms': result['gpu']['main']['time_ms'], 'cpu': result['cpu'], 'process_cpu_ms_per_frame': result['process_cpu_ms_per_frame'], 'rss_B': memory['Rss'], 'pss_B': memory['Pss'], 'allocator_resident_B': sum(int(block['resident_B']) for block in blocks), 'owners': owner}
        records.append(row)
...
output = {'baseline_commit': '373f989', 'candidate_commit': 'd1e10d4', 'layout': 'Yoga.Net', 'frames': 14000, 'physical_extent': list(next(iter(extents))), 'aggregation': 'pooled 1000 frames per variant and workload, two fresh processes, ABBA order; memory is median of two pre-GC boundaries', 'aggregate': aggregate, 'runs': records}
(root / 'chart-results.json').write_text(json.dumps(output, indent=2) + '\n')
