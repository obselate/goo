...
relation; additional error-message targets become descriptions. Modal overlays
must hide or disable their background semantics and manage focus, as with a
custom adapter; `Modal` describes the scope without changing Goo input policy.
...
- `new()`: creates an unattached adapter.
- `IsAvailable`: checks for a compatible native library without opening a window.
- `Update(AccessibilityTree)`: receives the owning window's retained tree.
- `Dispose()`: detaches and releases the native binding; repeated calls are safe.
...
Creates a selection-change request whose `caret` equals either `start` or
`start + length`. Invalid endpoints are rejected.
...
Gets the active UTF-16 selection endpoint.
