...
class DevToolsCell : Cell {
    ...
    private func BuildTopbar() Container {
        ...
        return Window.DragRegion(
            Container{
                Key: "topbar",
                Width: Length.Percent(100),
                Height: 36,
                FlexShrink: 0.0,
                Padding: EdgeLengths{Left: 16, Right: 8,},
                FlexDirection: FlexDirection.Row,
    ...
    private func BuildBody() Container {
        let panes = List[Blob]()
        if !compact || !showProperties {
            panes.Add(BuildCenter())
        }
        if compact && showProperties || !compact && !detailsHidden {
            if !compact {
                panes.Add(BuildDivider())
            }
    ...
    private func BuildDivider() Container -> Container{
        ...
        BackgroundColor: if resizing {
            DevToolsTheme.Accent
        } else {
            DevToolsTheme.Background
        },
        Hover: Style{BackgroundColor: DevToolsTheme.BorderStrong},
        OnPointerDown: (e PointerEvent) -> {
            if e.Button != PointerButton.Primary || resizing {
                return
            }
            e.Capture()
            e.PreventDefault()
        ...
        OnPointerMove: (e PointerEvent) -> {
            if resizing && e.PointerId == resizePointer {
                detailsWidth = Math.Clamp(
                    resizeStartWidth + resizeStartX - e.WindowPosition.X,
                    280.0,
                    Math.Max(280.0, layoutWidth - 266.0)
                )
                Rebuild()
            }
        },
        ...
        OnKeyDown: (e KeyEvent) -> {
            if e.Key == Key.Left || e.Key == Key.Right {
                detailsWidth = Math.Clamp(
                    InspectorWidth() + if e.Key == Key.Left {
                        16.0
                    } else {
                        -16.0
                    },
    ...
    private func BuildActions() Container {
        ...
        children.Add(
            Button{
                ...
                Text{
                    Key: "window-picker-label",
                    Content: "Inspect window",
                    FontSize: 12,
                    Color: DevToolsTheme.InkMuted
                },
                Text{
                    Key: "window-picker-current",
                    Content: session.SelectedWindow().Title,
                    Width: 0,
                    MinWidth: 0,
                    FlexGrow: 1.0,
                    FontSize: 12,
        ...
        children.Add(
            ActionButton(
                ...
                if !compact && !detailsHidden || compact && showProperties {
                    DevToolsTheme.SurfaceStrong
                } else {
                    DevToolsTheme.Background
                },
    ...
    private func BuildWindowPicker() Container {
        ...
        buttons.Add(
            Text{
                Key: "window-picker-help",
                FontSize: 12,
                Color: DevToolsTheme.InkMuted,
                TextWrap: TextWrap.Wrap,
                Content: if session.IsSample {
                    "Example windows. Choose one to explore its element tree."
                } else {
                    "Choose the app window whose elements you want to inspect."
                },
            }
        )
        var ordinal int32
    ...
    private func BuildInspectBanner() Container -> Container{
        ...
        Text{
            ...
            TextWrap: TextWrap.Wrap,
            Content: if session.IsSample {
                "Select an element below to inspect the sample."
            } else {
                "Click an element in the app window to inspect it."
            }
        },
    }
    ...
    private func BuildTreeSection() Container {
        ...
        var hiddenDepth = -1
        for row in visible {
            if session.Query == "" && hiddenDepth >= 0 && row.Depth > hiddenDepth {
                continue
            }
            hiddenDepth = -1
            rows.Add(BuildTreeRow(row, rowIndex))
            if session.Query == "" && collapsed.Contains(row.Node.Id) {
                ...
                Container{
                    ...
                    Text{
                        ...
                        Content: if session.State != DiagnosticConnectionState.Connected {
                            "Connect to an app to see its elements."
                        } else {
        ...
        return Container{
            ...
            Container{
                ...
                TextEntry{
                    ...
                    Width: Length.Percent(100),
                    Height: 30,
                    Padding: EdgeLengths{Left: 9, Right: 9,},
                    Value: session.Query,
    ...
    private func BuildTreeRow(row DiagnosticTreeRow, ordinal int32) Container {
                ...
                Button{
                    ...
                    DevToolsIcons.View(
                        ...
                        if collapsed.Contains(node.Id) {
                            DevToolsIcons.Chevron
                        } else {
                            DevToolsIcons.Expand
                        }
                    ),
                }
            )
        ...
        children.Add(
            Button{
                ...
                Text{
                    Key: "tree-type",
                    Content: if node.TypeName == node.DisplayName {
                        ""
                    } else {
                        node.TypeName
                    },
                    Width: Length.Percent(32),
                    FontSize: 11,
                    Color: DevToolsTheme.InkSubtle,
                    TextWrap: TextWrap.NoWrap,
                    TextTrimming: TextTrimming.Ellipsis
        ...
        return Container{
            ...
            Height: 34,
            MinHeight: 34,
            Padding: EdgeLengths{Left: Math.Min(float64(row.Depth) * 12.0, 72.0), Right: 6,},
            FlexDirection: FlexDirection.Row,
            AlignItems: AlignItems.Center,
            BackgroundColor: if selected {
                DevToolsTheme.Selection
            } else if node.Id == session.HoveredNodeId {
                DevToolsTheme.SurfaceRaised
            } else {
                DevToolsTheme.Background
            },
    ...
    private func BuildBottomPanel() Container {
        ...
        return Container{
            ...
            Container{
                ...
                Height: 38,
                MinHeight: 38,
                Padding: EdgeLengths{Left: 6, Right: 8,},
                FlexDirection: FlexDirection.Row,
                AlignItems: AlignItems.Center,
                Gap: 5,
                BorderBottomWidth: 1,
    ...
    private func BuildCaptureCard(screenshot DiagnosticScreenshot, ordinal int32) Container -> Container{
        ...
        Container{
            ...
            Text{Key: "capture-id", Content: screenshot.Id, FontSize: 11, Color: DevToolsTheme.InkSubtle,},
        },
        Text{Key: "capture-dimensions", Content: screenshot.Dimensions, FontSize: 12, Color: DevToolsTheme.Accent,},
        Text{
            Key: "capture-time",
    ...
    private func BuildInspector() Container {
        ...
        details.Add(
            Container{
                ...
                Width: Length.Percent(100),
                Margin: EdgeLengths{Top: 16},
                Gap: 8,
                BorderTopWidth: 1,
                BorderTopColor: DevToolsTheme.Border,
                Padding: EdgeLengths{Top: 8},
                FlexDirection: FlexDirection.Column,
                ActionButton(
                    "advanced-toggle",
                    if showAdvanced {
        ...
        return Container{
            ...
            Container{
                ...
                Height: 40,
                MinHeight: 40,
                Padding: EdgeLengths{Left: 12, Right: 6,},
                FlexDirection: FlexDirection.Row,
                AlignItems: AlignItems.Center,
                Gap: 8,
                BorderBottomWidth: 1,
    ...
    private func BuildDetails(node DiagnosticTreeNode) Container {
        ...
        return Container{
            ...
            FlexDirection: FlexDirection.Column,
            Gap: 5,
            Padding: EdgeLengths{Bottom: 14},
            Children: rows,
        }
    }

    private func Reported(value string) string -> if value == "" {
        "not reported by target"
    } else {
        value
    }
    ...
    private func SampleOrReported(value string, sample string) string -> if session.IsSample {
        sample
    } else {
        Reported(value)
    }
    ...
    private func DetailRow(key string, label string, value string) Container -> Container{
        ...
        Width: Length.Percent(100),
        MinHeight: 42,
        Padding: EdgeLengths{Left: 8, Right: 8, Top: 6, Bottom: 6,},
        FlexDirection: FlexDirection.Row,
    ...
    private func BuildOverridePanel() Container -> Container{
        ...
        Width: Length.Percent(100),
        Padding: 10,
        Margin: EdgeLengths{Top: 4, Bottom: 12,},
        FlexDirection: FlexDirection.Column,
        Gap: 7,
        BackgroundColor: DevToolsTheme.SurfaceRaised,
        BorderRadius: 5,
        ...
        Container{
            ...
            Text{
                ...
                FontWeight: 600,
                LetterSpacing: 0,
                Color: if session.OverrideActive {
                    DevToolsTheme.Accent
                } else {
                    DevToolsTheme.InkSubtle
                },
            ...
            Text{
                Key: "override-status",
                Content: if session.OverrideActive {
                    "active"
                } else {
                    "temporary"
                },
