...
class DiagnosticWire {
    shared {
        ...
        func ParseMessage(line string) DiagnosticMessage? {
            if line == nil || line.Trim().Length == 0 {
                return nil
            }
            try {
                ...
                let payloadRoot = if payload == "" {
                    root
                } else {
                    ParseElement(payload)
                }
                let effectiveCommand = if command == "" {
                    Text(payloadRoot, []string{"command", "method"})
                } else {
                    command
                }
        ...
        func ParseSnapshot(payload string, fallbackWindowId string, fullDefault bool) DiagnosticWireSnapshot? {
            if payload == nil || payload.Trim().Length == 0 {
                return nil
            }
            try {
        ...
        func ParseLog(payload string, kind DiagnosticMessageKind) DiagnosticWireLog {
            try {
                ...
                return DiagnosticWireLog{
                    ...
                    Level: if level == "" {
                        if kind == DiagnosticMessageKind.InputTrace {
                            "debug"
                        } else {
                            "info"
                        }
        ...
        func ParseCapture(payload string) DiagnosticWireCapture {
            try {
                ...
                let resolvedDimensions = if dimensions != "" {
                    dimensions
                ...
                let bytes = if data == "" {
                    ...
                    if Text(root, []string{"path"}) == data {
                        "file: " + data
                    } else {
                        data.Length.ToString() + " base64 chars"
                    }
                }
        ...
        private func MessageKind(messageType string, command string, payload JsonElement) DiagnosticMessageKind {
            let value = if messageType == "" {
                command
            } else {
                messageType
            }
            if value.Equals("hello", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.Hello
            }
            if value.Equals("snapshot", StringComparison.OrdinalIgnoreCase)
            || value.Equals("tree.snapshot", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.Snapshot
            }
            ...
            || value.Equals("tree.delta", StringComparison.OrdinalIgnoreCase) {
                return if Bool(payload, "full", false) {
                    DiagnosticMessageKind.Snapshot
                } else {
                    DiagnosticMessageKind.TreeDelta
                }
            }
            if value.Equals("selection", StringComparison.OrdinalIgnoreCase)
            || value.Equals("select", StringComparison.OrdinalIgnoreCase)
            || value.Equals("inspect.select", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.Selection
            }
            if value.Equals("hover", StringComparison.OrdinalIgnoreCase)
            || value.Equals("inspect.hover", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.Hover
            }
            if value.Equals("override", StringComparison.OrdinalIgnoreCase)
            || value.Equals("property.override", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.PropertyOverride
            }
            if value.Equals("input", StringComparison.OrdinalIgnoreCase)
            || value.Equals("input.trace", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.InputTrace
            }
            if value.Equals("log", StringComparison.OrdinalIgnoreCase)
            || value.Equals("logs", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.Log
            }
            if value.Equals("capture", StringComparison.OrdinalIgnoreCase)
            || value.Equals("screenshot", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.Screenshot
            }
            if value.Equals("window.added", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.WindowAdded
            }
            if value.Equals("window.removed", StringComparison.OrdinalIgnoreCase) {
                return DiagnosticMessageKind.WindowRemoved
            }
        ...
        private func Snapshot(root JsonElement, fallbackWindowId string, fullDefault bool) DiagnosticWireSnapshot {
            ...
            if result.WindowId == "" {
                result.WindowId = fallbackWindowId
            }
            ...
            if let values = windows {
                if values.ValueKind == JsonValueKind.Array {
                    for value in values.EnumerateArray() {
                        ...
                        if let actual = parsed {
                            result.Windows.Add(actual)
                        }
                    }
                }
            }
            ...
            if let values = nodes {
                if values.ValueKind == JsonValueKind.Array {
                    for nodeValue in values.EnumerateArray() {
                        ...
                        if let actual = node {
                            result.Nodes.Add(actual)
                        }
                    }
                }
            }
            if result.Id == "" {
                result.Id = "remote-window"
            }
            return result
        }
        ...
        private func Node(value JsonElement, parentId string) DiagnosticWireNode? {
            ...
            let result = DiagnosticWireNode(
                ...
                PreferText(
                    ...
                    ObjectText(value, []string{"accessibility", "accessibilityState"})
                ),
            ...
            if result.DisplayName == "" {
                result.DisplayName = result.TypeName
            }
            ...
            if let ids = childIds {
                if ids.ValueKind == JsonValueKind.Array {
                    for childValue in ids.EnumerateArray() {
                        let childId = Scalar(childValue)
                        if childId != "" && !result.ChildIds.Contains(childId) {
                            result.ChildIds.Add(childId)
                        }
                    }
                }
        ...
        private func PreferText(synthesized string, provided string) string -> if synthesized == "" {
            provided
        } else {
            synthesized
        }
        ...
        private func CapabilityText(root JsonElement, payload JsonElement) string {
            ...
            if values == nil {
                values = Element(payload, []string{"capabilities", "features"})
            }
            if let actual = values {
                if actual.ValueKind == JsonValueKind.Array {
                    var result = ""
        ...
        private func Text(root JsonElement, names[]string) string {
            ...
            if let actual = value {
                ...
                || actual.ValueKind == JsonValueKind.False {
        ...
        private func Number(root JsonElement, names[]string) int64 {
            ...
            if let actual = value {
                if actual.ValueKind == JsonValueKind.Number {
                    if actual.TryGetInt64(out var number) {
                        return number
                    }
                }
                if actual.ValueKind == JsonValueKind.String {
        ...
        private func Bool(root JsonElement, name string, fallback bool) bool {
            ...
            if let actual = value {
                ...
                if actual.ValueKind == JsonValueKind.String {
                    if Boolean.TryParse(actual.GetString(), out var parsed) {
                        return parsed
                    }
                }
            }
...
class DiagnosticPipeTransport : DiagnosticTransport {
    ...
    func Send(message DiagnosticMessage) bool {
        let line = if message.Raw == "" {
            DiagnosticWire.Request(message.Command, message.Payload)
        } else {
            message.Raw
        }
        try {
            lock gate {
                if state != DiagnosticConnectionState.Connected || writer == nil {
                    return false
                }
                if let active = writer {
                    active.WriteLine(line)
                }
            }
            return true
        } catch (error Exception) {
        ...
        try {
            ...
            while true {
                ...
                if let value = message {
                    ...
                    Enqueue(value)
                    pollSnapshot = value.IsResponse &&
                        value.Succeeded &&
                        (value.Command == "snapshot" || value.Command == "tree.snapshot")
                }
                if pollSnapshot {
                    Thread.Sleep(100)
                    lock gate {
            ...
            lock gate {
                if let attached = stream {
                    if current == nil || Object.ReferenceEquals(attached, current) {
                        stream = nil
                    }
                }
                reader = nil
    ...
    private func Handshake() string -> "{\"type\":\"hello\",\"protocol\":\"goo.devtools/1\",\"client\":\"goo-devtools\",\"version\":\"0.6.0\",\"capabilities\":[\"tree\",\"properties\",\"layout\",\"events\",\"logs\",\"accessibility\",\"capture\",\"source-navigation\",\"hot-reload\"]}"
    ...
    private func UpdateCapabilities(message DiagnosticMessage) {
        ...
        lock gate {
            ...
            for value in values {
                ...
                if item == "tree.delta" || item == "deltas" {
                    next.TreeDeltas = true
                }
                if item == "properties" {
                ...
                if item == "logs" || item == "log" {
                    next.Logs = true
                }
                if item == "runtime-overrides" || item == "override" {
                    next.RuntimeOverrides = true
                }
                if item == "capture" || item == "capture.rgba8" || item == "screenshots" {
                    next.Screenshots = true
                }
                if item == "input" || item == "input-trace" {
                    next.InputTrace = true
                }
                if item == "accessibility" {
                    next.Accessibility = true
                }
                if item == "source-navigation" {
                    next.SourceNavigation = true
                }
                if item == "hot-reload" {
                    next.HotReload = true
                }
...
class DiagnosticDisconnectedTransport : DiagnosticTransport {
    ...
    prop State DiagnosticConnectionState {
        get -> DiagnosticConnectionState.Disconnected
    }
    prop Endpoint DiagnosticEndpoint {
        get -> endpoint
    }
    prop Capabilities DiagnosticCapabilities {
        get -> DiagnosticCapabilities{}
    }
    prop IsSample bool {
        get -> false
    }
    ...
    shared {
        ...
        func Scan(projectDirectory string) List[DiagnosticEndpoint] {
            ...
            for directory in directories {
                ...
                } catch (_ IOException) { } catch (_ UnauthorizedAccessException) { }
            }
            return result
        }
        ...
        private func Read(path string) DiagnosticEndpoint? {
            try {
                ...
                && !protocol.Equals("1", StringComparison.OrdinalIgnoreCase) {
                    return nil
                }
