...
class StudioCell : Cell {
    ...
    private func selector(label string, index int32) Button {
        ...
        let background = if selectedStyle {
            GalleryTheme.Ink
        } else {
            GalleryTheme.SurfaceRaised
        }
        let foreground = if selectedStyle {
    ...
    private func writeStudioParameters(effect ShaderEffect, time float64, bounds ElementRect) {
        let playValue = if playing {
            1.0F
        } else {
            0.0F
        }
        effect.SetParameter(0, Vector4(float32(time), 960.0F, 540.0F, playValue))
        effect.SetParameter(
            1,
            Vector4(
                float32(pointerX),
        ...
        if selected == 0 {
            ...
            effect.SetParameter(3, Vector4(float32(Math.Clamp(c0 / 72.0, 0.25, 1.35)), float32(c1), float32(c2), 0.0F))
        } else if selected == 1 {
            effect.SetParameter(
                2,
                Vector4(float32(c0 * Math.PI / 180.0), float32((pointerY - 0.5) * 0.35), float32(c1), float32(c2))
            )
        } else if selected == 2 {
            ...
            while index < 3 {
                ...
                effect.SetParameter(
                    2 + index,
                    Vector4(float32(ringX[index]), float32(ringY[index]), float32(age), float32(amplitude))
                )
                index = index + 1
            }
        } else if selected == 5 {
        ...
        let body = Container{
            ...
            Container{
                ...
                Container{
                    ...
                    Container{Key: "studio-palettes", FlexDirection: FlexDirection.Row, Gap: 8, Children: palettes,},
                },
                Container{
                    Key: "studio-controls",
                    FlexDirection: FlexDirection.Row,
