...
func Main() {
    Window.ConfigureApplication("Goo Gallery", "0.5.4", "io.github.obselate.goo.gallery")
...
func RunSmoke() {
    ...
    while sizeIndex < 4 {
        ...
        || rootBox.Height < expectedHeight - 2.0 {
            throw InvalidOperationException(
                "Goo Gallery root does not fit window "
        ...
        || showcaseBox.Y + showcaseBox.Height > rootBox.Y + rootBox.Height + 1.0 {
            throw InvalidOperationException(
                "Goo Gallery showcase does not fit at "
                + sizes[sizeIndex].ToString()
            )
        }
    ...
    while index < root.ShowcaseCount() {
        if root.CurrentShowcase() != index {
            throw InvalidOperationException(
                "Goo Gallery pager skipped showcase "
                + index.ToString()
            )
        }
    ...
    Console.WriteLine(
        "gallery-smoke: sizes=2 layout=fit input=topbar+workers+rail showcases=9 pager=forward-back routes=4 shaderFrames=180 glassWindow=1 close=2"
    )
}
...
func MeasureRun(window Window, samples[]float64, spot string) int32 {
    ...
    return if median > 17.5 || p95 > 20.0 { 1 } else { 0 }
}
