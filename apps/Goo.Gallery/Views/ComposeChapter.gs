...
open class ComposeChapter : Cell[ComposeChapterInput], IDisposable {
        ...
        while left < results.Count - 1 {
            ...
            while right < results.Count {
                let before = results[right].Rank < results[left].Rank ||
                    results[right].Rank == results[left].Rank &&
                    results[right].Index < results[left].Index
                if before {
    ...
    private func buildTiles() List[Blob] {
        ...
        let layoutWidth = if orientation % 2 == 0 {
            34.0
        } else {
            21.0
        }
        let layoutHeight = if orientation % 2 == 0 {
            21.0
        } else {
            34.0
        }
            ...
            Container{
                ...
                Container{
                    ...
                    Container{
                        ...
                        Bottom: 16,
                        ZIndex: 3,
                        Padding: EdgeLengths{Left: 10, Right: 10, Top: 6, Bottom: 6,},
                        BackgroundColor: acid,
                        BorderWidth: 1,
                        BorderColor: ink,
