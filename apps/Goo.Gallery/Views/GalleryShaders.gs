...
class ShaderLabCell : Cell {
    ...
    private func frameValue() float32 -> if Playing {
        1.0F
    } else {
        0.0F
    }
    ...
    private func writeFrame(
        ...
        let down = if pointerDown {
            1.0F
        } else {
            0.0F
        }
        effect.SetParameter(0, Vector4(float32(time), 960.0F, 540.0F, frameValue()))
        effect.SetParameter(
            1,
            Vector4(
    ...
    private func buildChrome(programs GalleryShaderPrograms, time float64) Container {
        ...
        writeFrame(effect, ChromeCanvas, time, chromePointerX, chromePointerY, chromePointerPressure, chromePointerDown)
        effect.SetParameter(2, Vector4(float32(chromeYaw), float32(chromePitch), float32(roughness), float32(morph)))
        return Container{
            Key: if effectsReady() {
                "chrome-live"
            } else {
    ...
    private func buildRadial(programs GalleryShaderPrograms, time float64) Container {
        ...
        writeFrame(effect, RadialCanvas, time, radialPointerX, radialPointerY, radialPointerPressure, radialPointerDown)
    ...
    private func buildRipple(programs GalleryShaderPrograms, time float64) Container {
        ...
        writeFrame(effect, RippleCanvas, time, ripplePointerX, ripplePointerY, ripplePointerPressure, ripplePointerDown)
        var index int32 = 0
        while index < 3 {
            ...
            effect.SetParameter(
                2 + index,
                Vector4(float32(rippleX[index]), float32(rippleY[index]), float32(age), float32(amplitude))
            )
            index = index + 1
        }
            ...
            Container{
                ...
                Container{
                    ...
                    Container{
                        ...
                        BorderColor: Edges[Color]{Bottom: Color.Rgba(226, 238, 247, 34)},
    ...
    private func buildDither(programs GalleryShaderPrograms, time float64) Container {
        ...
        writeFrame(effect, DitherCanvas, time, ditherPointerX, ditherPointerY, 0.0, ditherPointerDown)
        effect.SetParameter(
            2,
            Vector4(float32(ditherYaw), float32(ditherPitch), float32(ditherShape), float32(ditherLevels))
        )
        return Container{
            Key: if effectsReady() {
                "dither-live"
    ...
    private func selectedSpecimen(programs GalleryShaderPrograms, time float64, out name string, out hint string) Blob {
        if Showcase == 0 {
            ...
            if effectsReady() {
                wolf = Cell.Mount[World3DCell](
                    "world-3d-live",
                    func (cell World3DCell) {
    ...
    private func buildContent(programs GalleryShaderPrograms) Container {
        ...
        if controls.Count > 0 {
            bodyChildren.Add(
                Container{
                    ...
                    MinWidth: if Compact || Showcase == 7 {
                        controlsMinWidth
                    } else {
                        controlsWidth
                    },
                    FlexShrink: 0.0,
                    FlexDirection: if Showcase == 7 && !Compact {
                        FlexDirection.Row
                    } else {
        ...
        let body = Container{
            ...
            FlexShrink: 1.0,
            MinHeight: 0,
            FlexDirection: if Compact || Showcase == 7 {
                FlexDirection.Column
            } else {
                FlexDirection.Row
            },
