...
public class GlassTerminalCell : Cell {
    ...
    private func currentTintName() string {
        ...
        return if tintIndex == 5 { "Amethyst" } else { "Smoke" }
    }
    ...
    private func blurValue() float32 {
        ...
        return if blurLevel == 2 { 0.62F } else { 0.24F }
    }
    ...
    private func buildQuickCornerToggle() Container {
        ...
        let toggleLabel = if isSquare {
            "⬚ SQUARE (0px)"
        } else {
            "◯ ROUND (" + cornerRadius.ToString("F0") + "px)"
        }
        let btnRadius = if isSquare {
            2
        } else {
            6
        }
    ...
    private func terminalLine(content string, color Color) Container -> Container(){Text{
            Content: content,
            FontFamily: "monospace",
            FontSize: 12,
            LineHeight: 1.4,
    ...
    private func terminalBadgeLine(content string, color Color) Container -> Container(){Text{
            Content: content,
            FontFamily: "monospace",
            FontSize: 12,
            FontWeight: 600,
    ...
    private func buildFloatingControlDock() Container {
        ...
        return Container{
            ...
            Container{
                ...
                dockButton(
                    "8px",
                    MathF.Abs(cornerRadius - 8.0F) < 0.1F,
                    btnRadius,
                    func () {
                        setCornerRadius(8.0F, "Sleek (8px)")
                    }
                ),
                dockButton(
                    "12px",
                    MathF.Abs(cornerRadius - 12.0F) < 0.1F,
                    btnRadius,
                    func () {
                        setCornerRadius(12.0F, "Native (12px)")
                    }
                ),
                dockButton(
                    "16px",
                    MathF.Abs(cornerRadius - 16.0F) < 0.1F,
                    btnRadius,
                    func () {
                        setCornerRadius(16.0F, "Modern (16px)")
                    }
                ),
                dockDivider(),
                // Opacity section
    ...
    internal prop CornerRadius float32 {
        get -> cornerRadius
    }
    internal prop Opacity float32 {
        get -> opacity
    }
    internal prop TintIndex int32 {
        get -> tintIndex
    }
    internal prop Effect ShaderEffect {
        get -> effect
    }
}
