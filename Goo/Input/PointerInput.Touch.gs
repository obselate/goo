...
internal partial class PointerInput {
  private func beginTouchPan(x float32, y float32) {
    ...
    if currentDevice != PointerDevice.Touch || !isSemanticPrimary()
      || captureTarget != nil || dragEntry != nil || dragEditor != nil { return }
    for var i = pressChain.Count; i > 0; i-- {
      let n = pressChain[i - 1]
  ...
  private func updateTouchPan(root Node?, resolver Resolver, x float32, y float32,
    prevented bool) bool{
      ...
      if prevented || captureTarget != nil || activeDragMatches() || dragEntry != nil
        || dragEditor != nil || !nodeVisibleInTree(tree, state.Target, false)
