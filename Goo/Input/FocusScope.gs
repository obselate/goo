...
internal class FocusScopeStack {
  ...
  internal func Refresh(root Node?, resolver Resolver, text TextInput) {
    for var i = entries.Count - 1; i >= 0; i-- {
    ...
    if !text.NativeFocusAllowed || owner.IsInputBlocked { return }
    if restorePending {
      let target = restore
      let previousRoot = restoreFrom
      let focused = text.FocusedNode()
    ...
    if !active.Entered || focused == nil || !FocusScopes.Contains(active.Root, focused)
      || !canReceiveInput(focused) {
        ...
        ? initial : text.FirstScopeFocus(active.Root)
        text.SetFocus(resolver, target)
      }
  }
