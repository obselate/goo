...
This extension is a thin VS Code front end for the `goo` CLI.
...
code --install-extension goo-devtools-0.5.4.vsix
```
...
Set `goo.cliPath` and `goo.project` in workspace settings when the defaults do not apply. The status item polls `goo doctor --json` and shows whether a live endpoint exists.
