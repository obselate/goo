...
`goo dev --input` enables diagnostics and input in the launched app. Add `--no-watch` to run without watching. `goo input <event> --pid PID --window NAME` then exercises that window. When launching outside the CLI, set `GOO_DEVTOOLS_INPUT=1` together with `GOO_DEVTOOLS=1`. See the [input protocol](../../docs/devtools/protocol.md#application-input) for coordinates, node IDs, modifiers, capture cancellation, acknowledgement, and error handling.
